...
そのまま盤面に出す。
    ...
    層          周期            役割                        技術
    ─────────────────────────────────────────────────────────────────
    ラダー      毎 tick         ゲートの開閉、インタロック  PLC ラダーロジック
    シーケンサ  毎 tick         事故対応・修理の段取り      逐次ステートマシン
    VLA         25 tick         パドルの連続制御            行動列を出すモデル
    VLM         150 tick        狙う区画・修理の可否判断    言語で判断するモデル

**下の層ほど速く、上の層ほど賢い。** 抽象度が違う以上この順序は動かないので、
上の層が古い情報で喋ることは前提として設計する。

## 時間の扱い

tick は論理時間で、実時間と結びつけない。`StackConfig.slowmo` で工程の時定数を
いくらでも伸ばせる — ラインを遅く回せば遅い判断でも間に合う、という現場の当たり前を
そのまま持ち込むためのつまみ。

これは逃げではなく前提の置き方で、**どの機械で回したかを結果から追い出す**ためにある。
「モデルが何秒かかったか」を測りたいのではなく、
**タクトを合わせて上位が参加できるようにしたとき、上位が何を寄与するか**を見たい。
実行環境の速さは、その問いに対しては雑音でしかない。
